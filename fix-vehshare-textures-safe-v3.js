...
function encodeSegmentFlags(size) {
  if (size < 0) throw new Error(`Dimensiune negativa RSC7: ${size}`);
  for (let ss = 0; ss <= 15; ss++) {
    const paddedSize = 0x20000 * (2 ** ss);
    if (paddedSize >= size) {
      const flags = ((1 << 4) | ss) >>> 0;
      return { flags, paddedSize, pageCount: 1 };
    }
  }
...
function parseYtd(buffer, label) {
  ...
  const pagesInfoPtr = ptrOffset(sys.readBigUInt64LE(0x08));
  return {
    version: rsc.version,
    dictVft,
    textures: entries,
    sys: Buffer.from(sys),
    gfx: Buffer.from(gfx),
    pagesInfoOffset: pagesInfoPtr.segment === 5 ? pagesInfoPtr.offset : null,
  };
}

function buildYtdPreservingLayout(parsed, additions) {
  const oldSys = parsed.sys;
  const oldGfx = parsed.gfx;
  const all = [
    ...parsed.textures.map((t) => ({ name: t.name, hash: joaat(t.name), old: t })),
    ...additions.map((t) => ({ name: t.name, hash: joaat(t.name), add: t })),
  ].sort((a, b) => a.hash - b.hash);

  for (let i = 1; i < all.length; i++) {
    if (all[i].hash === all[i - 1].hash) {
      throw new Error(`Hash duplicat in dictionar: "${all[i - 1].name}" / "${all[i].name}"`);
    }
  }
  if (all.length > 0xffff) throw new Error(`Prea multe texturi in dictionar: ${all.length}`);

  let sysCursor = alignUp(oldSys.length, 16);
  const ptrArrayOff = sysCursor;
  sysCursor += all.length * 8;
  const hashesOff = alignUp(sysCursor, 16);
  sysCursor = hashesOff + all.length * 4;

  const newStructOff = new Map();
  const newNameOff = new Map();
  for (const e of all) {
    if (!e.add) continue;
    sysCursor = alignUp(sysCursor, 16);
    newStructOff.set(e, sysCursor);
    sysCursor += TEXTURE_STRUCT_SIZE;
    sysCursor = alignUp(sysCursor, 4);
    newNameOff.set(e, sysCursor);
    sysCursor += Buffer.byteLength(e.name, 'utf8') + 1;
  }

  const sys = Buffer.alloc(sysCursor);
  oldSys.copy(sys);
  let gfxCursor = oldGfx.length;
  const newDataOff = new Map();
  for (const e of all) {
    if (!e.add) continue;
    gfxCursor = alignUp(gfxCursor, 0x1000);
    newDataOff.set(e, gfxCursor);
    gfxCursor += e.add.data.length;
  }
  const gfx = Buffer.alloc(gfxCursor);
  oldGfx.copy(gfx);
  ...
  for (let i = 0; i < all.length; i++) {
    ...
    let structOff;
    if (e.old) {
      structOff = e.old.structOffset;
      if (!Number.isInteger(structOff) || structOff < 0 || structOff + TEXTURE_STRUCT_SIZE > oldSys.length) {
        throw new Error(`Offset invalid pentru structura originala ${e.name}`);
      }
    } else {
      structOff = newStructOff.get(e);
      e.add.struct.copy(sys, structOff, 0, TEXTURE_STRUCT_SIZE);
      sys.writeBigUInt64LE(BigInt(SYS_BASE + newNameOff.get(e)), structOff + 0x28);
      sys.writeBigUInt64LE(BigInt(GFX_BASE + newDataOff.get(e)), structOff + 0x70);
      sys.write(e.name, newNameOff.get(e), 'utf8');
      e.add.data.copy(gfx, newDataOff.get(e));
    }
    sys.writeBigUInt64LE(BigInt(SYS_BASE + structOff), ptrArrayOff + i * 8);
    sys.writeUInt32LE(e.hash, hashesOff + i * 4);
  }

  sys.writeBigUInt64LE(BigInt(SYS_BASE + hashesOff), 0x20);
  sys.writeUInt16LE(all.length, 0x28);
  sys.writeUInt16LE(all.length, 0x2a);
  sys.writeBigUInt64LE(BigInt(SYS_BASE + ptrArrayOff), 0x30);
  sys.writeUInt16LE(all.length, 0x38);
  sys.writeUInt16LE(all.length, 0x3a);

  if (parsed.pagesInfoOffset != null && parsed.pagesInfoOffset + 10 <= sys.length) {
    sys.writeUInt8(1, parsed.pagesInfoOffset + 8);
    sys.writeUInt8(1, parsed.pagesInfoOffset + 9);
  }
  return buildRsc(parsed.version, sys, gfx).file;
}
...
function buildLibrary(vehshareDir, log) {
  const found = [];
...
function processResource(resourceDir, library, options, log) {
  ...
  for (const action of actions) {
    ...
    let parsed = null;
    if (!creating) {
      parsed = parseYtd(fs.readFileSync(targetPath), targetPath);
    ...
    if (!options.dryRun) {
      const rebuilt = creating
        ? buildYtd(merged, buildOptions)
        : buildYtdPreservingLayout(parsed, action.additions.map((a) => a.texture));
...
Optiuni:
  ...
  const library = buildLibrary(vehshareDir, log);
